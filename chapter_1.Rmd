...
```{r}
#Square resovoir with inflow labeled "birth" and outflow labeled "death"
grViz("
...
2. Suppose you wish to build a model for a lake that is fed by one river and three smaller streams that drain a 200 square mile watershed. Your goal is to determine how the turbidity of the lake water will change during a lO0-year rainstorm (i.e., a rainstorm so severe that it is expected to occur only once every 100 years). Define two different reservoirs that you would use in a systems model. In addition, identify the flow processes that affect each reservoir. list the reservoirs and their flows and specify the units of each. Do not try to identify any converters or mathematical relationships. Draw a system diagram for each reservoir, similar to Figures 1.1 and 1.2.
...
#Units
#1. Lake - volume of water (m^3)
#2. Watershed - area (m^2)
#3. Rainfall - volume of water (m^3)
#4. Evaporation - volume of water (m^3)
#5. Runoff - volume of water (m^3)
#6. Infiltration - volume of water (m^3)
#7. Streamflow - volume of water (m^3)
#8. Groundwater flow - volume of water (m^3)
#9. Sedimentation - volume of water (m^3)
#10. Turbidity - volume of water (m^3)
...
b) Renewal = RenewalRate * IslandResources
...
c) IslandResources(t + dt) = IslandResources(t) + Renewal - Depletion
justification: The island resources at time t + dt is equal to the island resources at time t plus the renewal minus the depletion. This is because the island resources at time t + dt is equal to the island resources at time t plus the renewal minus the depletion.
d) Depletion = DepletionRate * IslandResources
