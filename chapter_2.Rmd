...
```{r}
Oscillation <- function(t, y, params) {
  with(as.list(c(y, params)), {
    dC <- G * R - D
    dR <- W - Q * C
    return(list(c(dC, dR)))
...
parms <- c(G = 1, D = 20, Q = 1, W = 15)
times <- seq(0, 25, by = .1)
state <- c(C = 15, R = 20)

output <- lsoda(state, times, Oscillation, parms)
...
ggplot(as.data.frame(output)) +
  ...
  labs(title = paste("Consumer Dynamics; Oranism Growth Rate = ", parms[1], "; Organism Death Rate = ", parms[2],
       ...
       x = "Time (hours)", y = "Consumer (organisms)")

ggplot(as.data.frame(output)) +
    geom_line(aes(x = time, y = R), color = "red") +
    labs(title = paste("Consumer Dynamics; Oranism Growth Rate = ", parms[1], "; Organism Death Rate = ", parms[2],
                       "; Resource Consumption Rate = ", parms[3], "; Resource Growth Rate = ", parms[4]),
         x = "Time (hours)", y = "Resource (units)")
```
...
```{r}
Oil <- function(t, y, params) {
  with(as.list(c(y, params)), {
    dFV <- (VProductionRate*(POS/1e7) - VObsolescenceRate)
    dPOS <- (OProdRate*(URCO/1e8) - OConsRate * FV) 
    dURCO <- -OProdRate 
...
state <- c(FV = 1e6, POS = 1e7, URCO = 1e8)
parms <- c(VProductionRate = 1e4, VObsolescenceRate = 1e3, OProdRate = 1e5, OConsRate = 0.1)
out <- ode(y = state, times = times, func = Oil, parms = parms)
